...
export function HrExpenses() {
  ...
  return <Table title="All Expense Claims" search={search} onSearch={setSearch} rows={data?.items.map((x) => [x.employeeName, x.title, money.format(x.totalAmount), <StatusBadge status={x.status} kind="expense" />, <ExpenseActions status={x.status} onApprove={() => approve.mutate(x.id)} onReject={() => reject.mutate(x.id)} onPaid={() => paid.mutate(x.id)} />]) ?? []} />;
}
...
type UserFormErrors = Partial<Record<'firstName' | 'lastName' | 'email' | 'department' | 'designation' | 'password' | 'confirmPassword', string>>;
...
function UserDrawer({ user, onClose, onSave }: { user: User | null; onClose: () => void; onSave: (body: CreateUser | UpsertUser) => void }) {
  ...
  return <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm"><aside className="fixed right-0 top-0 z-50 flex h-full w-full max-w-full flex-col bg-white shadow-2xl transition-transform duration-300 dark:bg-slate-900 sm:max-w-lg">
    ...
    <div className="grid flex-1 gap-4 overflow-y-auto px-6 py-6">
      <Field label="Employee ID"><input className="input" placeholder="Auto Employee ID" value={form.employeeId} onChange={(e) => set('employeeId', e.target.value)} /></Field>
      ...
      <Field label="Manager"><select className="input" value={form.managerId ?? ''} onChange={(e) => set('managerId', e.target.value || null)}><option value="">No manager</option>{managerOptions?.map((m) => <option key={m.id} value={m.id}>{m.firstName} {m.lastName}</option>)}</select></Field>
      {!user && <>
