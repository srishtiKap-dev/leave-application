...
public sealed class UsersController(IPortalService portal, UserManager<ApplicationUser> users, IBlobStorageService blobs) : BaseApiController
{
    ...
    public async Task<ActionResult<ApiResponse<UserDto>>> Create([FromBody] UpsertUserRequest request)
    {
    ...
    [Authorize(Roles = "HRAdmin,SuperAdmin"), HttpPut("{id:guid}")] public async Task<ActionResult<ApiResponse<UserDto>>> Update(Guid id, [FromBody] UpdateProfileRequest request, CancellationToken ct) => OkResponse(await portal.UpdateProfileAsync(id, request, ct));
