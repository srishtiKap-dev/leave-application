...
public sealed class HrController(IPortalService portal) : BaseApiController
{
    ...
    [HttpPost("leave-balances/adjust")] public ActionResult<ApiResponse<object>> Adjust([FromBody] AdjustBalanceRequest request) => OkResponse<object>(new { request.UserId, request.Reason }, "Adjustment captured.");
    ...
    [HttpPost("public-holidays")] public async Task<ActionResult<ApiResponse<PublicHolidayDto>>> AddHoliday([FromBody] UpsertHolidayRequest request, CancellationToken ct) => OkResponse(await portal.UpsertHolidayAsync(null, request, ct));
