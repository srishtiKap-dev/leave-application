...
public sealed class LeaveTypesController(IPortalService portal) : BaseApiController
{
    ...
    [Authorize(Roles = "HRAdmin,SuperAdmin"), HttpPost] public async Task<ActionResult<ApiResponse<LeaveTypeDto>>> Create(UpsertLeaveTypeRequest request, CancellationToken ct) => OkResponse(await portal.UpsertLeaveTypeAsync(null, request, ct));
    [Authorize(Roles = "HRAdmin,SuperAdmin"), HttpPut("{id:guid}")] public async Task<ActionResult<ApiResponse<LeaveTypeDto>>> Update(Guid id, UpsertLeaveTypeRequest request, CancellationToken ct) => OkResponse(await portal.UpsertLeaveTypeAsync(id, request, ct));
}
