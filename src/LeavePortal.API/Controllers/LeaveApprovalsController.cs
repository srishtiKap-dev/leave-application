...
{
    [HttpGet("pending")] public async Task<ActionResult<ApiResponse<PagedResult<LeaveApplicationDto>>>> Pending([FromQuery] string? search, CancellationToken ct) => OkResponse(await portal.GetLeavesAsync(null, UserId, "Pending", null, null, search, Page(), ct));
    [HttpGet("all")] public async Task<ActionResult<ApiResponse<PagedResult<LeaveApplicationDto>>>> All([FromQuery] string? search, CancellationToken ct) => OkResponse(await portal.GetLeavesAsync(null, UserId, null, null, null, search, Page(1, 100), ct));
    ...
    [HttpGet("team-calendar")] public async Task<ActionResult<ApiResponse<PagedResult<LeaveApplicationDto>>>> Calendar(CancellationToken ct) => OkResponse(await portal.GetLeavesAsync(null, UserId, null, DateTime.UtcNow.Year, null, null, Page(1, 200), ct));
}
