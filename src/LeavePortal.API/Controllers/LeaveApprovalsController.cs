...
public sealed class LeaveApprovalsController(IPortalService portal) : BaseApiController
{
    ...
    [HttpPost("{id:guid}/approve")] public async Task<ActionResult<ApiResponse<LeaveApplicationDto>>> Approve(Guid id, ApprovalRequest request, CancellationToken ct) => OkResponse(await portal.ApproveLeaveByManagerAsync(id, UserId, request.Remarks, ct));
    [HttpPost("{id:guid}/reject")] public async Task<ActionResult<ApiResponse<LeaveApplicationDto>>> Reject(Guid id, ApprovalRequest request, CancellationToken ct) => OkResponse(await portal.RejectLeaveAsync(id, UserId, request.Remarks, false, ct));
