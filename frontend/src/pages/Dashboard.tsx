...
export function Dashboard() {
  ...
  return <div className="mx-auto grid w-full max-w-7xl gap-5 sm:gap-6">
    <div className="mb-1 sm:mb-2"><h1 className="text-xl font-bold text-slate-900 dark:text-slate-100 sm:text-2xl">Employee Dashboard</h1><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Track your leave balances, recent claims, and approvals.</p></div>
...
function BalanceCard({ code, remaining, total }: { code: string; remaining: number; total: number }) {
  ...
  return <div className="card">
    ...
    <p className="mt-5 text-sm font-medium text-slate-500 dark:text-slate-400">{code}</p>
    <p className="mt-1 text-3xl font-bold text-slate-950 dark:text-white">{remaining}</p>
    <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">of {total} days remaining</p>
    <div className="mt-5 h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className={`h-full ${bar}`} style={{ width: `${percent}%` }} /></div>
  </div>;
}
